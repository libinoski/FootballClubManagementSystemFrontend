...
const ClubViewAllSuspendedPlayers = () => {
        ...
        const fetchSuspendedPlayers = async () => {
            ...
            try {
                ...
                const response = await axios.post(
                    'http://localhost:4040/api/club/clubViewAllSuspendedPlayers',
                    { clubId },
                    {
                        headers: {
                            token
                ...
                if (response.status === 200) {
                    setSuspendedPlayers(response.data.data);
                }
            } catch (error) {
                if (error.response) {
    ...
    const handleViewDetails = (playerId) => {
        ...
        navigate('/clubViewOneSuspendedPlayer');
    };
        ...
        <div>
            ...
            <div style={{ background: 'linear-gradient(to right, #000000, #000000)', color: '#fff', minHeight: '100vh', paddingTop: '70px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                {isLoading ? (
                    ...
                    <div className="text-center mt-4 alert alert-warning">No suspended players available.</div>
                )}
            </div>
            <CommonFooter />
        </div>
...
export default ClubViewAllSuspendedPlayers;
