            ...
            <div className="d-flex align-items-center justify-content-center" style={{ minHeight: '100vh' }}>
                <form onSubmit={handleSubmit} noValidate className="needs-validation" style={{ maxWidth: '500px', width: '100%' }}>
                    ...
                    <div className="mb-3 position-relative">
                        <select className={`form-select ${validationErrors.clubId ? 'is-invalid' : ''}`} id="clubDropdown" name="clubId" value={playerData.clubId} onChange={handleClubChange} required>
                            <option value="">Select Club *</option>
                            {clubs.map(club => (
                                <option key={club.clubId} value={club.clubId}>{club.clubName}</option>
                            ))}
                        </select>
                        {validationErrors.clubId && <div className="invalid-feedback">{validationErrors.clubId}</div>}
                    </div>
                    <div className="mb-3">
