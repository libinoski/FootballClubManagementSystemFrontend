...
const AdminViewOneMatch = () => {
        ...
        const fetchMatchDetails = async () => {
            ...
            try {
                ...
                const response = await axios.post(
                    'http://localhost:4040/api/admin/adminViewOneMatch',
                    { adminId, matchId },
                    {
                        headers: {
                            token
    ...
    const handleEndOneMatch = async () => {
        try {
            const token = sessionStorage.getItem('token');
            const adminId = sessionStorage.getItem('adminId');
            const matchId = sessionStorage.getItem('matchId');
            const response = await axios.post(
                'http://localhost:4040/api/admin/adminEndOneMatch',
                { adminId, matchId },
                {
                    headers: {
                        token
                            ...
                            {matchDetails ? (
                                ...
                                <h5 className="card-title text-center mb-4" style={{ fontSize: '28px', fontWeight: 'bold', color: '#ffffff' }}>{matchDetails.matchName}</h5>
                                    ...
                                    <div className="d-flex justify-content-center pb-4">
                                        <button className="btn btn-danger" onClick={handleEndOneMatch}>End Match</button>
                                    </div>
                                </div>
                            ) : (
...
export default AdminViewOneMatch;
