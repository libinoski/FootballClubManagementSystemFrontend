...
const AdminViewOneEndedMatch = () => {
        ...
        const fetchMatchDetails = async () => {
            ...
            try {
                ...
                const response = await axios.post(
                    'http://localhost:4040/api/admin/adminViewOneEndedMatch',
                    { adminId, matchId },
                    {
                        headers: {
                            token
    ...
    const handleAdMatchPoint = (matchId) => {
        sessionStorage.setItem('matchId', matchId);
        navigate('/adminAddMatchPoint');
    };
                        ...
                        <div className="card border-0" style={{ background: 'rgba(255, 255, 255, 0.1)', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)', backdropFilter: 'blur(20px)', borderRadius: '10px' }}>
                            {matchDetails ? (
                                <div className="card-body">
                                    <h5 className="card-title text-center mb-4" style={{ fontSize: '28px', fontWeight: 'bold', color: '#007bff' }}>{matchDetails.matchName}</h5>
                                    ...
                                    <div className="d-flex justify-content-center pb-4">
                                        <button className="btn btn-danger ms-2" onClick={() => handleAdMatchPoint(matchDetails.matchId)}>Add Points</button>
                                    </div>
                                </div>
                            ) : (
...
export default AdminViewOneEndedMatch;
